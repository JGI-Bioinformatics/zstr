...
void decompress_files(const std::vector<std::string>& file_v, const std::string& output_file) {
  ...
  for (const auto& f : file_v) {
    ...
    //
    std::unique_ptr<std::istream> is_p = (f != "-" ? std::unique_ptr<std::istream>(new zstr::bgzf_ifstream(f)) :
...
bool verify_random_access(const std::string& f) {
  ...
  while (is_verified) {
    if (bytes == 0) {
      ...
      bytes = raw.gcount();
      if (raw.eof()) {
        std::cerr << "Found natural end at " << raw.tellg() << std::endl;
        break;
      }
      if (bytes > 0) {
        is_verified = false;
        std::cerr << "Found unnatural end at " << raw.tellg() << std::endl;
        break;
      }
      break;
    }
    if (raw.fail()) {
      is_verified = false;
      std::cerr << "Got failure reading BGZF header" << std::endl;
      break;
    }
    if (bytes != 18) {
      is_verified = false;
      std::cerr << "Did not read full BGZF header" << std::endl;
      break;
    }
    if (memcmp(buf, zstr::BGZF_MAGIC_HEADER, zstr::BGZF_BLOCK_HEADER_LENGTH - 2) != 0) {
      is_verified = false;
      std::cerr << "Block header mismatch at " << raw.tellg() << std::endl;
      break;
    }
    ...
    if (bytes < zstr::BGZF_BLOCK_FOOTER_LENGTH) {
      is_verified = false;
      std::cerr << "Did not read full BGZF footer" << std::endl;
      break;
    }
    ...
    if (raw.fail()) {
      is_verified = false;
      std::cerr << "Got failure reading BGZF footer" << std::endl;
      break;
    }
  }
  ...
  //#pragma omp parallel reduction(& : is_verified)
  {
    std::ostringstream oss;
    zstr::bgzf_ifstream in(f);
    auto test_vfp = in.get_bgzf_virtual_file_pointer();
    assert(test_vfp.to_int_id() == in.tellg());
    ...
    assert((size_t)in.compressed_tellg() == cmp_pos);

    in.compressed_seekg(0);
    std::ostringstream().swap(oss);
    // std::cerr << "try @0 again in.tellg()=" << in.tellg() << " uncomp.tellp()=" << oss.tellp() << std::endl;
    assert((size_t)oss.tellp() == 0);
    assert((size_t)in.compressed_tellg() == 0);
    cat_stream(in, oss);
    // std::cerr << "in.tellg()=" << in.tellg() << " uncomp.tellp()=" << oss.tellp() << std::endl;
    assert((size_t)oss.tellp() == uncmp_pos);
    assert((size_t)in.compressed_tellg() == cmp_pos);
    ...
    auto last_i = idx.size();
    //#pragma omp for
    for (size_t blk_i = 0; blk_i < idx.size(); blk_i++) {
      const auto& blk = idx[blk_i];
      // for (zstr::bgzf_index::iterator iter = idx.begin(); iter != idx.end(); iter++) {
      //  const auto& blk = *iter;
      ...
      // assert(vfp == vfp2);
      // std::cerr << "Found vfp=" << vfp.get_file_offset() << " at " << vfp.get_block_offset() << " after pos=" << blk.caddr <<
      // std::endl;
      if (vfp.get_file_offset() != blk.caddr) {
        is_verified = false;
        std::cerr << "Expected first byte of block did not match!" << std::endl;
        blk_i = idx.size();
        continue;  // instead of break which is forbidden in openmp
      }
      if (vfp.get_block_offset() != 0) {
        is_verified = false;
        std::cerr << "Expected first byte of block did not match block offset!" << std::endl;
        blk_i = idx.size();
        continue;  // instead of break which is forbidden in openmp
      }

      if (last_i != idx.size() && blk.caddr > idx[last_i].caddr) {
        // test 1 over previous block
        std::vector<size_t> attempts;
        attempts.push_back(idx[last_i].caddr + 1);
        attempts.push_back((idx[last_i].caddr + idx[last_i].caddr) / 2);
        attempts.push_back(idx[last_i].caddr - 1);
        for (auto attempt : attempts) {
          auto test_vfp = in.find_next_bgzf_block(attempt);
          if (test_vfp != vfp) {
            is_verified = false;
            std::cerr << "Expected find within previous block to find this one!" << std::endl;
            blk_i = idx.size();
            continue;  // instead of break which is forbidden in openmp
          }
        }
      ...
      test_vfp = in.get_bgzf_virtual_file_pointer();
      assert(test_vfp.to_int_id() == in.tellg());
      // std::cerr << "Got " << (vfp==test_vfp ? "correct" : "INCORRECT") << " vfp: " << test_vfp.get_file_offset() << " at " <<
      // test_vfp.get_block_offset() << std::endl;
      assert(vfp == test_vfp);
      // std::cerr << "Checking block " << blk_i << " at cmp_offset " << vfp.get_file_offset() << " at " << vfp.get_block_offset()
      // << std::endl;
      cat_stream(in, oss);
      // std::cerr << "in.tellg()=" << in.tellg() << " uncomp.tellp()=" << oss.tellp() << std::endl;
      if ((uint64_t)oss.tellp() != uncmp_pos - blk.uaddr) {
        is_verified = false;
        std::cerr << "Got wrong remainder of uncompressed stream. expected " << uncmp_pos - blk.uaddr << " got " << oss.tellp()
                  << " blk.uaddr=" << blk.uaddr << std::endl;
        blk_i = idx.size();
        continue;  // instead of break which is forbidden in openmp
      }
      ...
      if (oss.tellp() > 0) {
        ...
        assert((uint64_t)oss.tellp() == 0);
        cat_stream(in, oss);
        if ((uint64_t)oss.tellp() != uncmp_pos - blk.uaddr - 1) {
          is_verified = false;
          std::cerr << "2Got wrong remainder of uncompressed stream. expected " << uncmp_pos - blk.uaddr << " got " << oss.tellp()
                    << std::endl;
          blk_i = idx.size();
          continue;  // instead of break which is forbidden in openmp
        }
...
void compress_files(const std::vector<std::string>& file_v, const std::string& output_file) {
  ...
  std::unique_ptr<std::ostream> os_p =
      (not output_file.empty() ? std::unique_ptr<std::ostream>(new zstr::bgzf_ofstream(output_file)) :
...
int main(int argc, char* argv[]) {
  ...
  while ((c = getopt(argc, argv, "h?")) != -1) {
    switch (c) {
      ...
      case 'h':
        ...
        break;
      default: usage(std::cerr, argv[0]); std::exit(EXIT_FAILURE);
    }
  }
  //
  ...
  int ret = 0;
  for (auto& f : file_v) ret |= verify_random_access(f) ? 0 : 1;
  return ret;
}
